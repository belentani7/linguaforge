  ...
  return (
    <div className={dark ? "app-shell dark-mode" : "app-shell"}>
      <header className="topbar">
        <div className="brand" onClick={() => goTo("dashboard")} role="button" tabIndex={0} onKeyDown={(event) => (event.key === "Enter" || event.key === " ") && goTo("dashboard")}>
        ...
        <main className="main-content">
          <div className="content-wrap">
            {active === "dashboard" && <>
              ...
              <div className="metric-grid"><Metric icon={Flame} value="7 días" label="Racha actual" accent="coral" /><Metric icon={Zap} value="1.240" label="XP acumulada" accent="gold" /><Metric icon={BookOpen} value="18" label="Lecciones hechas" accent="indigo" /><Metric icon={Target} value="B1" label="Nivel actual · ES" accent="teal" /></div>
              ...
              <div className="section-heading"><div><p className="eyebrow">CONTINÚA TU RUTA</p><h2>Lecciones recomendadas</h2></div><button className="text-link" onClick={() => goTo("languages")}>Ver mapa completo <ArrowRight size={15} /></button></div><div className="lesson-grid">{LESSONS.map((lesson) => <Card className="lesson-card" key={lesson.title} role="button" tabIndex={0} onKeyDown={(event) => (event.key === "Enter" || event.key === " ") && goTo("lesson")} onClick={() => goTo("lesson")}><div className={`lesson-card-icon ${lesson.color}`}><lesson.icon size={20} /></div><div className="lesson-card-body"><Badge variant="outline">{lesson.module}</Badge><h3>{lesson.title}</h3><p>{lesson.time} · {lesson.xp}</p></div><ChevronRight size={18} className="lesson-arrow" /></Card>)}</div>
            </>}

            {active === "languages" && <><div className="page-heading"><div><p className="eyebrow">MIS IDIOMAS</p><h1>Elige tu <em>próximo mundo.</em></h1><p className="subheading">Cada idioma abre una nueva forma de mirar.</p></div><div className="pair-route"><span>Ruta bidireccional</span><Select value={sourceLanguage} onValueChange={setSourceLanguage}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{availableLanguages.map((language) => <SelectItem key={language.code} value={language.code}>{language.native}</SelectItem>)}</SelectContent></Select><ArrowRight size={15} /><Select value={target} onValueChange={setTarget}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{availableLanguages.filter((language) => language.code !== sourceLanguage).map((language) => <SelectItem key={language.code} value={language.code}>{language.native}</SelectItem>)}</SelectContent></Select><small>{availablePaths.length ? "Ruta activa" : "Preparando ruta"}</small></div></div><div className="language-grid">{availableLanguages.map((language, index) => <Card className={`language-card ${index < 2 ? "selected" : ""}`} key={language.code} role="button" tabIndex={0} onKeyDown={(event) => (event.key === "Enter" || event.key === " ") && (setTarget(language.code), goTo("dashboard"))} onClick={() => { setTarget(language.code); goTo("dashboard"); }}><div className="language-card-top"><LanguageMark language={language} size="lg" />{index < 2 ? <Badge className="status-badge">En curso</Badge> : <Badge variant="outline">Explorar</Badge>}</div><h3>{language.native}</h3><p>{language.name} · {language.code.toUpperCase()}</p>{index < 2 ? <><div className="language-card-progress"><Progress value={index === 0 ? 62 : 18} /><span>{index === 0 ? "B1" : "A1"}</span></div><small>{index === 0 ? "62% del nivel actual" : "Comenzando"}</small></> : <div className="language-cta">Iniciar diagnóstico <ArrowRight size={15} /></div>}</Card>)}</div><Card className="diagnostic-banner"><div className="diagnostic-illustration"><Target size={32} /></div><div><p className="eyebrow">NUEVO IDIOMA</p><h2>Empieza con una evaluación que te entiende.</h2><p>El diagnóstico adapta la ruta a lo que ya sabes, sin hacerte repetir lo obvio.</p></div><Button className="button-dark" onClick={() => setDiagnosticStep(0)}>Hacer diagnóstico <ArrowRight size={16} /></Button></Card></>}

            {active === "practice" && <><div className="page-heading"><div><p className="eyebrow">PRÁCTICA LIBRE</p><h1>Entrena lo que <em>necesitas.</em></h1><p className="subheading">Sesiones aleatorias para reforzar cualquier área de tu ruta.</p></div><div className="practice-streak"><Flame size={18} />7 días de racha</div></div><Card className="practice-panel"><div className="practice-filters"><div><label htmlFor="practice-language">Idioma</label><Select value={target} onValueChange={setTarget}><SelectTrigger id="practice-language"><SelectValue /></SelectTrigger><SelectContent>{availableLanguages.map((language) => <SelectItem key={language.code} value={language.code}>{language.native}</SelectItem>)}</SelectContent></Select></div><div><label htmlFor="practice-level">Nivel</label><Select value={practiceFilter} onValueChange={setPracticeFilter}><SelectTrigger id="practice-level"><SelectValue /></SelectTrigger><SelectContent>{["Todos", ...LEVELS.map((level) => level.code)].map((level) => <SelectItem key={level} value={level}>{level}</SelectItem>)}</SelectContent></Select></div><div><label htmlFor="practice-topic">Tema</label><Select defaultValue="Todos"><SelectTrigger id="practice-topic"><SelectValue /></SelectTrigger><SelectContent>{["Todos", "Vida cotidiana", "Viajes", "Trabajo", "Conversación"].map((topic) => <SelectItem key={topic} value={topic}>{topic}</SelectItem>)}</SelectContent></Select></div></div><div className="practice-divider" /><div className="practice-start"><div className="practice-orbit"><BrainCircuit size={33} /></div><div><h2>Una práctica hecha para hoy</h2><p>10 ejercicios · Vocabulario y conversación · {practiceFilter === "Todos" ? "Todos los niveles" : practiceFilter}</p></div><Button className="button-coral" onClick={() => goTo("exercise")}>Comenzar <ArrowRight size={16} /></Button></div></Card><div className="section-heading compact"><div><p className="eyebrow">FORMATOS</p><h2>Entrena cada destreza</h2></div></div><div className="skill-grid">{[{ icon: BookOpen, title: "Vocabulario", desc: "Palabras que se quedan", color: "teal" }, { icon: PencilLine, title: "Gramática", desc: "Estructuras que fluyen", color: "indigo" }, { icon: Volume2, title: "Pronunciación", desc: "Sonar con naturalidad", color: "coral" }, { icon: Headphones, title: "Conversación", desc: "Usar el idioma de verdad", color: "gold" }].map((item) => <Card key={item.title} className="skill-card" role="button" tabIndex={0} onKeyDown={(event) => (event.key === "Enter" || event.key === " ") && goTo("exercise")} onClick={() => goTo("exercise")}><div className={`skill-icon ${item.color}`}><item.icon size={20} /></div><h3>{item.title}</h3><p>{item.desc}</p><ArrowRight size={16} /></Card>)}</div></>}

            {active === "review" && <><div className="page-heading"><div><p className="eyebrow">REPETICIÓN ESPACIADA</p><h1>Recuerda más, <em>esfúerzate menos.</em></h1><p className="subheading">Tu memoria tiene un ritmo. Nosotros lo seguimos.</p></div><div className="review-count"><strong>24</strong><span>tarjetas pendientes</span></div></div><Card className="review-hero"><div className="review-hero-icon"><BrainCircuit size={28} /></div><div><p className="eyebrow">SESIÓN DE HOY</p><h2>24 tarjetas listas para repasar</h2><p>Una sesión de 8 minutos mantiene activas tus palabras más importantes.</p><div className="review-stats"><span><strong>12</strong> nuevas</span><span><strong>12</strong> para reforzar</span></div></div><Button className="button-dark" onClick={() => goTo("review-session")}>Repasar ahora <ArrowRight size={16} /></Button></Card><div className="section-heading compact"><div><p className="eyebrow">TU RITMO</p><h2>Memoria en movimiento</h2></div></div><div className="memory-grid"><Card><CardHeader><CardTitle>Retención estimada</CardTitle><CardDescription>Últimos 30 días</CardDescription></CardHeader><CardContent><div className="retention-number">86<span>%</span></div><Progress value={86} className="tall-progress" /><p className="muted-text">+8% frente al mes anterior</p></CardContent></Card><Card><CardHeader><CardTitle>Próximas revisiones</CardTitle><CardDescription>Distribución de tu cola</CardDescription></CardHeader><CardContent><div className="review-bars"><div><span>Hoy</span><Progress value={72} /><strong>24</strong></div><div><span>Mañana</span><Progress value={43} /><strong>14</strong></div><div><span>Esta semana</span><Progress value={28} /><strong>8</strong></div></div></CardContent></Card></div></>}
