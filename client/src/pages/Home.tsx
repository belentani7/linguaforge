      ...
      <div className="workspace">
        ...
        <main className="main-content">
          <div className="content-wrap">
            ...
            {active === "profile" && <ProfileView userName={user?.name ?? "Alex"} target={target} targetCodes={targetCodes} languages={availableLanguages} onTargetChange={(code) => { setTarget(code); setTargetCodes((current) => current.includes(code) ? current : [...current, code]); }} onTargetsChange={setTargetCodes} onSave={() => profileUpdate.mutate({ name: user?.name ?? "Alex", nativeLanguageCode: "es", targetLanguageCodes: targetCodes }, { onSuccess: (result) => { setTargetCodes(result.targetLanguageCodes); toast.success("Preferencias guardadas"); } })} onFeedback={async (category, message) => { await feedbackMutation.mutateAsync({ category, message }); }} feedbackPending={feedbackMutation.isPending} feedbackError={feedbackMutation.error?.message} />}
          </div>
        </main>
...
function ProfileView({ userName, target, targetCodes, languages, onTargetChange, onTargetsChange, onSave, onFeedback, feedbackPending, feedbackError }: { userName: string; target: string; targetCodes: string[]; languages: CatalogLanguage[]; onTargetChange: (value: string) => void; onTargetsChange: (values: string[]) => void; onSave: () => void; onFeedback: (category: "lesson" | "exercise" | "accessibility" | "content" | "general", message: string) => Promise<void>; feedbackPending: boolean; feedbackError?: string }) {
