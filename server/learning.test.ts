...
describe("learning procedures", () => {
  it("derives the CEFR level from completed lessons", () => {
    expect([0, 4, 5, 9, 10, 19, 20, 34, 35, 54, 55].map(levelForCompletedLessons)).toEqual(["A1", "A1", "A2", "A2", "B1", "B1", "B2", "B2", "C1", "C1", "C2"]);
  });
  it("recommends a CEFR level from diagnostic skill scores", async () => {
    const caller = appRouter.createCaller(createContext());
    const result = await caller.diagnostic.complete({ targetLanguageCode: "es", scores: { vocabulary: 80, grammar: 75, comprehension: 72, communication: 73 } });
  ...
  it("creates an empty but typed SRS queue until cards are imported", async () => {
    const caller = appRouter.createCaller(createContext());
    const result = await caller.srs.queue({ targetLanguageCode: "en", limit: 12 });
    expect(result).toEqual({ targetLanguageCode: "en", dueCount: 0, cards: [] });
  });
});
