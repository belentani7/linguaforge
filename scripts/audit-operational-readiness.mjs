...
const baseUrl = (process.env.LINGUAFORGE_PREVIEW_URL ?? "http://127.0.0.1:3000").replace(/\/$/, "");
...
const blockedGates = {
  ...
  externalAnalytics: process.env.LINGUAFORGE_ANALYTICS_EXTERNAL_ENABLED !== "true",
};
...
function bytesUnder(root) {
  ...
  return fs.readdirSync(root).reduce((sum, entry) => sum + bytesUnder(path.join(root, entry)), 0);
}
...
try {
  ...
  health = { ok: response.ok && body?.result?.data?.json?.ok === true, status: response.status };
} catch (error) {
  health = { ok: false, status: 0, error: error instanceof Error ? error.message : String(error) };
}
...
const result = {
  baseUrl,
  health,
  build: { exists: fs.existsSync(distRoot), bytes: buildBytes, megabytes: Number((buildBytes / 1024 / 1024).toFixed(2)) },
  blockedGates,
...
if (!health.ok || !result.build.exists || Object.values(blockedGates).some((value) => !value)) process.exitCode = 1;
