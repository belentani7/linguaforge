...
async function connect() {
  ...
  socket.addEventListener("message", (event) => { const message = JSON.parse(event.data); const resolver = pending.get(message.id); if (resolver) { pending.delete(message.id); message.error ? resolver.reject(new Error(JSON.stringify(message.error))) : resolver.resolve(message.result); } });
...
const failures = results.flatMap((result) => result.findings.map((finding) => ({ route: result.route, ...finding })));
console.log(JSON.stringify({ baseUrl, routes: results, failureCount: failures.length, failures, passed: failures.length === 0 }, null, 2));
socket.close();
