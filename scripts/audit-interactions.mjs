...
async function walk(dir) {
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    lines.forEach((line, index) => {
      ...
      const context = lines.slice(Math.max(0, index - 20), Math.min(lines.length, index + 3)).join(" ");
...
await walk(root);
console.log(JSON.stringify({ checkedRoot: root, eventTypes: ["onClick", "onKeyDown", "onMouseDown", "onPointerDown", "onKeyUp", "onChange", "onSubmit"], findings }, null, 2));
